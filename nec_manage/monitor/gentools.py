...
def generate_spider(jsonfile):
    ...
    ok = spider_template.format(spider_name=arr[0], name=arr[1], redis_key=arr[2],
                                allowed_domains=arr[3], allow=arr[4], deny=arr[5],
                                title_xpath=arr[6], content_xpath=arr[7],
                                )
...
if __name__ == '__main__':
    test_code = """{"name":"caijing", "start_urls": ["http://www.caijing.com.cn"],
              "allowed_domains": ["caijing.com.cn"],"rule_allow": ["caijing.com.cn/\\\\d{8}/\\\\d*"],
              "rule_deny": [".*photos.*",".*politics.*"],"xpath_title": ["//*[@id='cont_title']/text()"],
